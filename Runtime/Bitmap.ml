...
value unsafe_set_point bm x y = do
{
  ...
  Bytes.set (Bytes.of_string bm.bm_data) i (Char.unsafe_chr (Char.code bm.bm_data.[i] lor get_bit x))
};
...
value unsafe_unset_point bm x y = do
{
  ...
  Bytes.set (Bytes.of_string bm.bm_data) i (Char.unsafe_chr (Char.code bm.bm_data.[i] land (lnot (get_bit x))))
};
