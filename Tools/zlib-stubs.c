...
CAMLprim value zlib_set_input(value stream, value string)
{
  ...
  if (!zs->in_buf) { zs->z.avail_in = 0; failwith("not enough memory"); }
...
CAMLprim value zlib_deflate_init(value buffer_size, value level)
{
  ...
  zs = Stream_val(block);
  if (deflateInit(&zs->z, Int_val(level)) != Z_OK) failwith (zs->z.msg);
  CAMLreturn(block);
}
...
CAMLprim value zlib_inflate_init(value buffer_size)
{
  ...
  block = alloc_zstream(Int_val(buffer_size));
  zs = Stream_val(block);
  if (inflateInit(&zs->z) != Z_OK) failwith (zs->z.msg);
  CAMLreturn(block);
}
...
CAMLprim value zlib_compress(value dest, value source)
{
  ...
  if (compress(String_val(dest), &len, String_val(source), string_length(source)) != Z_OK) failwith("compress");
...
CAMLprim value zlib_compress2(value dest, value source, value level)
{
  ...
  uLongf len = string_length(dest);
  if (compress2(String_val(dest), &len, String_val(source), string_length(source), Int_val(level)) != Z_OK) failwith("compress2");
  CAMLreturn(Val_int(len));
};
...
CAMLprim value zlib_uncompress(value dest, value source)
{
  CAMLparam2(dest, source);
  uLongf len = string_length(dest);
  if (uncompress(String_val(dest), &len, String_val(source), string_length(source)) != Z_OK) failwith("uncompress");
  CAMLreturn(Val_int(len));
};
